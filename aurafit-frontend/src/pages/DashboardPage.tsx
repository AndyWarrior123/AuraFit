...
import { Footprints, Flame, Droplets, Moon, Zap, RefreshCw } from 'lucide-react'
    ...
    <div className="min-h-screen p-4 md:p-6 pb-24 md:pb-6 space-y-5">
      ...
      >
        <div>
          <h1 className="text-xl font-bold text-white">
            {getGreeting()}, <span className="neon-text-purple">{user?.display_name}</span>
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {/* XP Ring + character */}
          <div
            className="glow-card rounded-2xl p-5 flex flex-col items-center gap-4"
          >
            {sheetLoading || !stats ? (
          ...
          {stats && (
            ...
            >
              ...
              <div className="flex items-end gap-1">
                <span className="text-2xl font-black neon-text-cyan" style={{ fontFamily: 'Orbitron, system-ui' }}>
              ...
              <div className="mt-2 h-1.5 rounded-full bg-white/5 overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  animate={{ width: `${Math.min((stats.daily_xp_earned / 500) * 100, 100)}%` }}
                  transition={{ duration: 1.2, delay: 0.5 }}
                />
              </div>
              <p className="text-xs text-slate-600 mt-1">Daily goal: 500 XP</p>
            </motion.div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {today && (
            <div className="grid grid-cols-4 gap-3">
              <StatPill icon={<Footprints size={16} />} value={today.total_steps.toLocaleString()} label="Steps" color="#22c55e" />
              <StatPill icon={<Flame size={16} />} value={today.total_calories_burned} label="Calories" color="#f97316" />
              <StatPill icon={<Droplets size={16} />} value={`${(today.total_water_ml / 1000).toFixed(1)}L`} label="Water" color="#06b6d4" />
              <StatPill icon={<Moon size={16} />} value={`${Math.round(today.sleep_minutes / 60 * 10) / 10}h`} label="Sleep" color="#a855f7" />
            </div>
          )}
          ...
          <div className="glow-card rounded-2xl p-5 flex flex-col items-center gap-3">
            ...
            <p className="text-xs text-slate-600 text-center">
              Just say what you did — <span className="text-cyan-400">"I ran 5km this morning"</span>
            </p>
          </div>
